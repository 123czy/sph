  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="item in tradeInfo.userAddressList" :key="item.id">
        <span @click="changeAddress(item.id)"  :class="{selected:item===addressInfo}">{{item.consignee}}</span>
        <p>
      ...
      <div class="detail" >
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="(detail,index)
        in tradeInfo.detailArrayList" :key="index">
          <li style="width: 80px;height: 80px">
            <img :src="detail.imgUrl" alt="" style="width: 100%;height: 100%">
          </li>
          <li>
            <p>
              {{detail.skuName}}</p>
...
import {mapState,mapActions} from 'vuex'
  export default {
    ...
    methods:{
     ...
     async  submitOrderFn(){
        const {code,data} = await this.submitOrder({tradeNo:this.tradeInfo.tradeNo,order:{
        ...
        // const {code} = await this.submitOrder({tradeNo:this.tradeInfo.tradeNo,order:{
      ...
      if(code === 200){
       ...
       changeAddress(id){
         this.addressInfo = this.$store.state.order.tradeInfo.userAddressList.find(item => item.id === id)
       }
    },
   async mounted(){
    ...
    watch:{
        "$store.state.order.tradeInfo":{
            ...
            handler(val){
                this.addressInfo = val.userAddressList.find(item => item.isDefault===`1`)
            },
            deep:true
        }
    }
