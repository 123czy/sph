<template>
  <div class="detail">
    ...
    <section class="con">
      <!-- 导航路径区域 -->
      <div class="conPoin">
        <span v-if="goodsDetail.categoryView">{{goodsDetail.categoryView.category1Name}}</span>
        <span v-if="goodsDetail.categoryView">{{goodsDetail.categoryView.category2Name}}</span>
        <span v-if="goodsDetail.categoryView">{{goodsDetail.categoryView.category3Name}}</span>
      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          v-if="goodsDetail.skuInfo">
            <h3 class="InfoName"> {{goodsDetail.skuInfo.skuName}}</h3>
            <p class="news">{{goodsDetail.skuInfo.skuDesc}}</p>
            <div class="priceArea">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(attr,attrIndex) in goodsDetail.spuSaleAttrList" :key="attr.id">
                <dt class="title"> {{attr.saleAttrName}}</dt>
                <dd changepirce="0" :class="{active:val.isChecked===`1`}" v-for="(val,valIndex) in attr.spuSaleAttrValueList" :key="val.id" @click="activeFn({attrIndex,valIndex})">{{val.saleAttrValueName}}</dd>
              </dl>  
            </div>
            <div class="cartWrap">
              <div class="controls">
...
import {mapActions,mapGetters,mapState} from "vuex";
  ...
  import {addSkuToCart} from "http/http"
  ...
  export default {
    ...
    async addCart(){
          //  try {
               const code = await this.addSkuToCart({skuid:this.id,skuNum:this.skuNum})
               if(code === 200){
                   // 通过localStorage携带商品的完整信息
                   window.sessionStorage.setItem("sph_skuInfo",JSON.stringify(this.goodsDetail.skuInfo))
                   //跳转到添加购物车成功界面  携带商品数量信息  商品的完整信息
